...
Carve DeepSeek-V4-Flash (~13B active) and Qwen A3B (~3B active) into visual expert sectors, forensically inspect what each sector contains, train specialist or **broad** adapters, and promote gated AetherPackages.
{: .fs-6 .fw-300 }

[Get started]({% link getting-started.md %}){: .btn .btn-primary .fs-5 .mb-4 .mb-md-0 .mr-2 }
[Flash-0731 guide]({% link guides/flash-0731.md %}){: .btn .fs-5 .mb-4 .mb-md-0 .mr-2 }
...
- miss fused expert banks entirely (DeepSeek-V4 Flash), or  
...
|------------|----------------|
| **Expert Group Studio** | Carve the lattice into ~active-fire sectors (~13B Flash / ~3B A3B) |
...
| **Flash-0731 native** | Fused `gate_up_proj`/`down_proj` via PEFT `target_parameters` |
...
{: .note }
Full bf16 retrain of ~284B is **not** the product goal. AetherForge trains **adapters (ESFT/LoRA)** on the full safetensors checkpoint, with optional selective full-expert updates.
...
| [Flash-0731]({% link guides/flash-0731.md %}) | Fused experts, validate-flash, PEFT |
