# Training DeepSeek-V4-Flash-0731 with AetherForge
...
- **Routed experts:** fused `DeepseekV4Experts` with 3D params:
  - `gate_up_proj` shape `(256, 4096, 4096)`
  - `down_proj` shape `(256, 4096, 2048)`
  - `is_transposed=False`
- **Shared:** `DeepseekV4MLP` with standard `gate_proj` / `up_proj` / `down_proj` Linears
...
1. Detect family `deepseek_v4_flash` from config / name
...
aetherforge remote launch --exec \
  -c configs/base.yaml -c configs/deepseek_v4_flash.yaml \
...
| Mode | Feasible? |
|------|-----------|
| Full bf16 train of 284B on one consumer GPU | **No** |
...
- [x] HF id resolves; `AutoConfig` → `DeepseekV4Config`
- [x] Weight index has `ffn.experts` × 43 layers × 256 experts
- [x] Meta `from_config` exposes `DeepseekV4Experts` + shapes
